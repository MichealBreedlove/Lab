...
ansible-playbook -i "$INVENTORY" "$PLAYBOOKS/00_preflight.yml" 2>&1 || echo "[WARN] Some hosts unreachable (continuing)"

echo ""
echo "=== Phase 2: Node BIOS/Firmware ==="
ansible-playbook -i "$INVENTORY" "$PLAYBOOKS/35_inventory_bios.yml" 2>&1 || echo "[WARN] BIOS inventory partial"

if [ "$MODE" = "full" ]; then
    echo ""
    echo "=== Phase 3: OPNsense Inventory ==="
    ansible-playbook -i "$INVENTORY" "$PLAYBOOKS/10_inventory_opnsense.yml" 2>&1 || echo "[WARN] OPNsense inventory failed"

    echo ""
    echo "=== Phase 4: Proxmox Inventory ==="
    ansible-playbook -i "$INVENTORY" "$PLAYBOOKS/20_inventory_proxmox.yml" 2>&1 || echo "[WARN] Proxmox inventory partial"

    echo ""
    echo "=== Phase 5: Switch Inventory ==="
    ansible-playbook -i "$INVENTORY" "$PLAYBOOKS/30_inventory_switches.yml" 2>&1 || echo "[WARN] Switch inventory partial"
fi
